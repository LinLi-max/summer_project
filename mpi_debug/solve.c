...
{   
    for(int i = 0; i < 4; i++)
    {
        MPI_Send(&U[e][0][i], (gy + 2), MPI_DOUBLE, right, 0, comm);
        MPI_Recv(&U[s-1][0][i], (gy + 2), MPI_DOUBLE, left, 0, comm, MPI_STATUS_IGNORE);

        MPI_Send(&U[s][0][i], (gy + 2), MPI_DOUBLE, left, 1, comm);
        MPI_Recv(&U[e+1][0][i], (gy + 2), MPI_DOUBLE, right, 1, comm, MPI_STATUS_IGNORE);
    }
}
