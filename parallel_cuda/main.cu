...
int main(int argc, char* argv[])
{
    ...
    call_init(d_U, dx, dy, gam, col, row, TPB);  //initialization
    call_cfl(d_U, dx, dy, dt, sf, gam, col, row);  //calculate dt using the cfl number

    while (current_time < total_time)
    {
        call_solver_2d(d_U, d_U_half, d_FG, d_temp, dx, dy, dt[0], gam, col, row, TPB);  //use a second order Lax-Wendroff two-step difference scheme
