...
{
    exchange(U, s, e, left, right, win);
    solve_x(U, U_half, F, temp, dx, dt / 2.0, gam, s, e, left, right, win);
...
void solve_x(double ***U, double ***U_half, double ***F, double ***temp, double dx, double dt, double gam, int s, int e, int left, int right, MPI_Win win)
{
    ...
    exchange(U, s, e, left, right, win); // a data exchange is required after correction U
